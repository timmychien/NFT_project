...
setInterval(function () {
  if (
    (parseInt(Date.now() / 1000) - 57600) % 86400 >= 0 &&
    (parseInt(Date.now() / 1000) - 57600) % 86400 < 60
  ) {
    pool.getConnection(function (err, connection) {
      connection.query(
        "UPDATE  member_info SET votecount=0",
        function (err, rows) {
          if (err) {
            console.log(err);
          } else {
        ...
        if (err) {
          ...
          for (var i = 0; i < rows_1.length; i++) {
            if (now >= rows_1[i].startVotestamp) {
              connection.query(
                "UPDATE  voting SET status=? where votingId=?",
                ["投票進行中", rows_1[i].votingId],
                function (err, rows_2) {
                  if (err) {
                    console.log(err);
                    //return;
